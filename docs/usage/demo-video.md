...
4. **上一关 Verified Known Knowns 较少时，下一关自动增加基础认知提升环节，并明确解释调整原因；**
5. AI 持续生成任务 Context、实现方案和 Goal vN；
6. Goal Forge 后交给 ChatGPT、Codex 或多 Agent 队伍继续执行、验证和交付。

视频同时展示每轮正反馈：四象限变化、认知分、科研目标进度、预计剩余时间和 Goal 版本变化。
...
发布分支使用一次性 GitHub Actions 工作流运行真实网页并调用 Playwright Chromium 原生录屏。由于二次 VP9 压缩在 GitHub runner 中存在不稳定性，本版本直接保留 Chromium 生成的原始 WebM，不让转码工具阻塞发布。
...
- 视频可以在主流浏览器中播放；
- 视频开场出现真实科研需求；
- 固定四象限、1–3 个问题、认知自适应、Context/Goal 与 Agent 执行均清晰可见；
