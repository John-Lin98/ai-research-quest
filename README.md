...
> **核心流程：先沟通，再执行。** Research Quest 先通过游戏关卡了解项目背景、已有材料、约束、用户偏好与 Known–Unknown 四象限；关键信息确认后，再冻结 `Context + Goal` 并交给 Codex 执行。Codex 不必从一句模糊命令猜测需求，而是依据更完整、精确、可追溯的上下文工作。
...
```text
真实科研需求与项目材料
→ 沟通阶段：游戏关卡 + Known–Unknown 四象限 + 用户偏好
...
Research Quest 将复杂科研协作分成两个清晰阶段：

- **Game Loop｜沟通与认知对齐**：AI 用每轮 1–3 个关键问题了解项目事实、用户理解、隐含经验和偏好；
...
1. AI 先读取真实需求、项目材料和已有提示词，展示完整关卡路线与预计时间；
2. 每回合只用 1–3 个关键问题补齐项目信息、用户偏好和 Known–Unknown 四象限；
3. 只有经过确认或验证的信息才进入 Frozen Context，开放未知和证据边界同时保留；
4. AI 将 Frozen Context 编译成包含输入、步骤、验收、Agent 分工和退出规则的 Codex Goal；
5. Codex 依据 Context + Goal 直接执行，减少重复询问、误解和目标漂移；
6. 执行结果回写四象限和 Goal，直到形成可验证交付。
