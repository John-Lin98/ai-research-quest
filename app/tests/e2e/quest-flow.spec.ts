...
test("完整 Dashboard 展示真实任务、四象限与简短公开边界", async ({ page }) => {
  ...
  await expect(page.getByRole("complementary", { name: "认知地图自适应提示" })).toContainText("现有认知地图中的已验证内容较少");
  await page.getByRole("group", { name: "回答关卡小测" }).getByRole("button").first().click();
  await expect(cognitionMap).toContainText("系统会根据选择、小测和任务结果自动补充证据");
  await expect(cognitionMap).toContainText("已验证");

  const metricsPanel = page.locator("section").filter({ has: page.getByRole("heading", { name: "四象限与 Verified 计分" }) });
...
test("真实需求面板与认知地图案例博文可访问", async ({ page }) => {
  ...
  await expect(caseStudy.getByRole("link", { name: "查看完整案例：认知地图怎样决定每轮问题" })).toHaveAttribute("href", "./case-study-alphafold-casp14.html");
  ...
  await expect(page.getByRole("heading", { name: "“为什么这一步最值得问”不是固定话术" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "每次选择都会成为真实输入" })).toBeVisible();
});
