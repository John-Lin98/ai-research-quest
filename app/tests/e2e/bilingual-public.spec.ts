...
const ENGLISH_DEMO_FILE = "/en/index.html";
...
test("英文 Demo 完整展示认知地图并支持逐轮选择", async ({ page }) => {
  // GitHub Pages 将 /en/ 解析为 /en/index.html；Vite 开发服务器测试时直接打开文件入口。
  await page.goto(ENGLISH_DEMO_FILE);
...
test("英文 Demo 与博文在当前视口不产生横向溢出", async ({ page }) => {
  for (const path of [ENGLISH_DEMO_FILE, "/en/case-study-alphafold-casp14.html"]) {
